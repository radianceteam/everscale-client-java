...
public class Processing {
 ...
 `wait_for_transaction`.
    ...
    * @param sendEvents Flag for requesting events sending
    */
    public CompletableFuture<ResultOfProcessMessage> processMessage(Abi.ParamsOfEncodeMessage messageEncodeParams, Boolean sendEvents, Consumer<ProcessMessageEvent> consumer) {
        return context.requestJSONCallback("processing.process_message", "{"+Stream.of((messageEncodeParams==null?null:("\"message_encode_params\":"+messageEncodeParams)),(sendEvents==null?null:("\"send_events\":"+sendEvents))).filter(_f -> _f != null).collect(Collectors.joining(","))+"}", consumer, ProcessMessageEvent.class)
            .thenApply(json -> TONContext.convertValue(json, ResultOfProcessMessage.class));
    }
