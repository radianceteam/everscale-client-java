...
public class AbiTest extends TestBase {
    ...
    public void encode_v2() throws Exception {
        ...
        DeploySet deploySet = new DeploySet(tvc, null, null);
        CallSet deployCallSet = new CallSet("constructor", new FunctionHeader(expire, time, keys.getPublic()), null);

        ResultOfEncodeMessage unsigned = abiModule.encodeMessage(abi, null, deploySet, deployCallSet, new Signer.External(keys.getPublic()), null).get();
        ...
        ResultOfAttachSignature signed = abiModule.attachSignature(abi, keys.getPublic(), unsigned.getMessage(), signature).get();
        ...
        ResultOfEncodeMessage signed2 = abiModule.encodeMessage(abi, null, deploySet, deployCallSet, new Signer.Keys(keys), null).get();
        assertEquals(signed.getMessage(), "");

        String address = "0:05beb555e942fa744fd96f45a9ea9d0a8248208ca12421947c06e59bc997d309";
        CallSet runCallSet = new CallSet("returnValue", new FunctionHeader(expire, time, null), "{\"id\":\"0\"}");

        ResultOfEncodeMessage unsigned2 = abiModule.encodeMessage(abi, address, null, runCallSet, new Signer.External(keys.getPublic()), null).get();
        ...
        ResultOfAttachSignature signed3 = abiModule.attachSignature(abi,keys.getPublic(), unsigned2.getMessage(), signature).get();
        ...
        ResultOfEncodeMessage signed4 = abiModule.encodeMessage(abi, address, null, runCallSet, new Signer.Keys(keys), null).get();
        ...
        ResultOfEncodeMessage noPubkey = abiModule.encodeMessage(abi, address, null, runCallSet, Signer.None, null).get();
